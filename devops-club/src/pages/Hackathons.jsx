    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md relative">
                ...
                {regStatus.message ? <p className={`p-3 rounded-md text-center ${regStatus.message.includes('successful') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{regStatus.message}</p>
                : <form onSubmit={handleRegSubmit} className="space-y-4">
                    ...
                    <button type="submit" disabled={regStatus.submitting} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold disabled:bg-gray-400 hover:bg-blue-700 transition-colors">{regStatus.submitting ? 'Submitting...' : 'Submit Registration'}</button>
                </form>}
            </div>
        </div>
    );
